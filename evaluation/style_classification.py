...
def test(test_data_path, model=None, tokenizer=None, model_path=None, tokenizer_path=None, num_class=2, batch_size=64):
    if model is None:
        model = StyleClassifier.from_pretrained(model_path, style_num=num_class)
    ...
    test_set = get_dataset_lst(test_data_path, mode="test")
    test_set = TransferDataset(test_set)
    test_data_loader = DataLoader(test_set, shuffle=True, batch_size=batch_size)
    #test_classifier(config, styleClassifier, tokenizer, test_loader)


    print("evaluating {}".format(test_data_path))
    ...
    print("acc:{}/{}, {}%".format(correct, len(test_set), correct *100. / len(test_set)))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--save_dir', type=str, default="/workspace/saved_models/yelp_roberta_cls/")
