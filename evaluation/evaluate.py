...
parser.add_argument("--model_dir",type=str, default="./saved_models/roberta_yelp_classifier_epoch_1")
...
def evaluate_style_acc(evaluation_files, pretrained_cls, tokenizer, batch_size=64):
    device = "cuda" if torch.cuda.is_available() else "cpu"

    pretrained_cls.eval()
    pretrained_cls.to(device)
    ...
    for batch_idx, batch in enumerate(test_loader):
        ...
        style_labels = batch['target_label'].to(device)
        original_labels = torch.ones(size=style_labels.shape).to(device) - style_labels

        transferred_inputs = tokenizer(batch['transferred_text'], padding=True, return_tensors="pt")
        transferred_input_ids = torch.tensor(transferred_inputs['input_ids']).long().to(device)
    
        reference_inputs = tokenizer(batch["reference_text"], padding=True, return_tensors="pt")
        reference_input_ids = torch.tensor(reference_inputs['input_ids']).long().to(device)
        
        original_inputs = tokenizer(batch["text"], padding=True, return_tensors="pt")
        original_input_ids = torch.tensor(original_inputs['input_ids']).long().to(device)
        ...
        trans_correct += transferred_outputs.eq(style_labels.to(device)).sum().item()
    
        reference_outputs = reference_outputs["logits"].argmax(dim=-1)
        ref_correct += reference_outputs.eq(style_labels.to(device)).sum().item()

        original_outputs = original_outputs["logits"].argmax(dim=-1)
        original_correct += original_outputs.eq(original_labels.to(device)).sum().item()
...
# def evaluate_transfer_results(file_path, classifier_path, tokenizer_path):
#     print('evaluating {}'.format(file_path))
#     with open(file_path, 'r') as f:
#             evaluation_files = json.load(f)

#     bleu_scores = evaluate_bleu(evaluation_files)
#     accs = evaluate_style_acc(
#         test_data_path=file_path, 
#         pretrained_cls, tokenizer, config)
...
if __name__ == "__main__":
    ...
    for result_file in result_files:
        ...
        #print("evaluating {}".format(results_file))

        bleu_scores = evaluate_bleu(evaluation_files)
        accs = evaluate_style_acc(evaluation_files, pretrained_cls, tokenizer, config)
